...
local function process_json(json)
	...
	local function scale_bitrate(br)
		...
		if counter >= 2 then
			return string.format("%.1fGbps", br)
		elseif counter >= 1 then
			return string.format("%.1fMbps", br)
		else
			return string.format("%.1fKbps", br)
		end
	end
	...
	local function format_table(formats, columns)
		...
		for _, f in ipairs(formats) do
			...
			for i, column in ipairs(show_columns) do
				...
				row = row
					.. (i > 1 and string.format("%" .. spacing .. "s", "") or "")
